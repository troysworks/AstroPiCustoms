...
def move_mount(button, scale):
    ...
    if button == 0:
        ...
        TrackerData.base.soft_dec_adder -= scale / TrackerData.base.dec_or_alt_pulse_per_deg
    return "0"
def guide_mount(button,msec):
    scale = msec/1000
    move_mount(button, scale)
    return "0"

def Sd_set_dec_deg(dms):
    # sDD*MM:SS
    # dms = '45*23:10'
    ...
    mm = int(temp1[0])
    ss = int(temp1[1])
    ddd = sdd + (mm * 0.01666)  + (ss * 0.0002778)
    if temp[0] < 0:
        ddd = ddd * -1
    temp_dec = ddd
    # TrackerData.base.dec_deg_decimal = ddd
    return "1"

def Sr_set_ra_hr(hms):
    # HH:MM:SS
    temp = hms.split(':')
    hrd = int(temp[0]) + (int(temp[1]) * 0.01666)  + (int(temp[2]) *  0.0002778)
    temp_ra = hrd
    # TrackerData.base.ra_hour_decimal = hrd
    return "1"
...
command_map = {
    ...
    ":GT#" : TrackerData.base.ra_az_osc_calc, #  Get tracking rate Returns: TT.T#    # Current Track Frequency expressed in hertz assuming a synchonous motor design where a 60.0 Hz motor clock
    ...
    ":GR": TrackerData.base.ra_hour_hms, #Get Telescope RA Returns: HH:MM.T# or HH:MM:SS# Depending which precision is set for the telescope

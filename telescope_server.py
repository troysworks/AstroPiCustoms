#!/usr/bin/env python
"""socket/IP server which listens for Meade LX200 style serial commands.

Intended to mimick a SkyFi (serial to socket/IP bridge) and compatible
...
# Main Server Code
# ================
# tdb http://mainecav.org/todd/astronomy/files/lx200gps_rs-232_autoguiding.pdf
# Meade’s Autostar lx200gps_rs-232
# Suite uses an undocumented command.
# Apparently, Meade has added the command
...
command_map = {
    ...
    # 0 Slew is Possible 1<string># Object Below Horizon w/string message 2<string># Object Below Higher w/string message
    ...
    ":MA#"  # Slew to target altitude and azimuth.Returns:Returns:0 no errorif the target is below the lower limit: the string“1Object Below Horizon #”if the target is above the high limit: the string“2Object Below Higher #”if the slew cannot be performed due to another cause: the string“3Cannot Perform Slew #”if the mount is parked: the string“4Mount Parked #”if the mount is restricted to one side of the meridian and the object is on the other side:the string“5Object on the other side #”
    #     :Me# Move east (for equatorial mounts) or left (for altazimuth mounts) at current rate.
    # Returns: nothing
    # If this command is given while moving at the guide rate, and the ultra precision mode is
    # not active, turning off the mount while this command is in effect will make the mount to
    # stay parked when turned on again.
    # :Mn# Move north (for equatorial mounts) or up (for altazimuth mounts) at current rate.
    # Returns: nothing
    # :Ms# Move south (for equatorial mounts) or down (for altazimuth mounts) at current rate.
    # Returns: nothing
    # :Mw# Move west (for equatorial mounts) or right (for altazimuth mounts) at current rate.
    # Returns: nothing
    # :MgeXXXX#
    # :MeXXX# Corrects the position of the mount to the east (for equatorial mounts) or left (for
    # altazimuth mounts) by an amount equivalent to a motion of XXX milliseconds at the
    # current autoguide speed. The maximum length of the correction is 1000 milliseconds up
    # to firmware revision 2.9.20. From firmware revision 2.10, the maximum length of the
    # correction is 9999 milliseconds.
    # :MgnXXXX#
    # :MnXXX# Corrects the position of the mount to the north (for equatorial mounts) or up (for
    # altazimuth mounts) by an amount equivalent to a motion of XXX milliseconds at the
    # current autoguide speed. The maximum length of the correction is 1000 milliseconds up
    # to firmware revision 2.9.20. From firmware revision 2.10, the maximum length of the
    # correction is 9999 milliseconds.
    # :MgsXXXX#
    # :MsXXX# Corrects the position of the mount to the south (for equatorial mounts) or down (for
    # altazimuth mounts) by an amount equivalent to a motion of XXX milliseconds at the
    # current autoguide speed. The maximum length of the correction is 1000 milliseconds up
    # to firmware revision 2.9.20. From firmware revision 2.10, the maximum length of the
    # correction is 9999 milliseconds.
    # :MgwXXXX#
    # :MwXXX# Corrects the position of the mount to the west (for equatorial mounts) or right (for
    # altazimuth mounts) by an amount equivalent to a motion of XXX milliseconds at the
    # current autoguide speed. The maximum length of the correction is 1000 milliseconds up
    # to firmware revision 2.9.20. From firmware revision 2.10, the maximum length of the
    # correction is 9999 milliseconds.
...
while True:
    ...
    # $ ./socat GOPEN:/dev/ptyp0,ignoreeof socket:raspberrypi8:4030
